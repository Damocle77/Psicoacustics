...
# ╭──────────────────────────────────────────────────────────────────────────────╮
# │ aegis_sonar_wide.sh — Gennaio (2026)                                         │
# │                                                                              │
# │ • Converte una traccia 5.1 in AC3/EAC3                                       │
# │ • DSP surround Sonar / Wide / Aegis (psicoacustico)                          │
...
──────────────────────────────────────────────────────────────────────────────────────────────
UTILIZZO:
  ./aegis_sonar_wide.sh <ac3|eac3> <si|no> [file|""] [bitrate] [sonar|wide|aegis|voice]
...
PARAMETRI:
  ...
  5) mode       sonar | wide | aegis | voice
               • sonar = “altezza” (simulazione psicoacustica 5.1.2 verticale)
               • wide  = "ampiezza" (simulazione psicoacustica 7.1 orizzontale)
               • aegis = "intermedia" (simulazione psicoacustica 5.1.2 cupola)
               • voice = "EQ Voce Sartoriale" su FC soltanto (no DSP surround)

ESEMPI (singolo file):
  ./aegis_sonar_wide.sh eac3 no  "film.mkv" 768k sonar 
  ./aegis_sonar_wide.sh ac3  si  "film.mkv" 640k wide 

ESEMPI (batch cartella: file=""):
  ./aegis_sonar_wide.sh eac3 no "" 448k wide  
  ./aegis_sonar_wide.sh ac3  si "" 640k sonar
──────────────────────────────────────────────────────────────────────────────────────────────
USAGE
...
[[ "$SUR_MODE" =~ ^(sonar|wide|aegis|voice)$ ]] || { err "Modalità: sonar|wide|aegis|voice"; exit 1; }
...
[SLlate_in]adelay=85,lowpass=f=1300,volume=0.45[SLlate];
...
[SRlate_in]adelay=85,lowpass=f=1300,volume=0.45[SRlate];
...
for CUR_FILE in "${FILES[@]}"; do
  ...
  # come da sorgenti surround → mappatura su SL/SR
