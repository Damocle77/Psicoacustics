...
# ╭──────────────────────────────────────────────────────────────────────────────╮
# │   convert_2AC3_dual.sh - SONAR Upfiring + Voice HQ sartoriale                │
# │                                                                              │
...
# │                                                                              │
# │   Parametri CLI:                                                             │
# │      ./convert_2ac3_dual.sh <mode> <keep> "<file>" <bitrate>                 │
# │                                                                              │
...
# │    • Boost Clean:  SL +2.3 dB / SR +2.3 dB                                   │ 
# │    • Boost Sonar: SL +2.8 dB / SR +2.8 dB                                    │
...
# │      4. Esegui: ./convert_2ac3_dual.sh sonar no "Film.mkv" 640k             │
...
────────────────────────────────────────────────────────────────────────────────
UTILIZZO:
  ./convert_2ac3_dual.sh <sonar|clean|dual> <si|no> [file.mkv/.mp4] [bitrate]
...
NOTE:
  ...
  • Boost Surround: Sonar SL +2.8 / SR +2.8 dB, Clean SL +2.3 / SR +2.3 dB.
...
[SLm][SLv][SLlate]amix=inputs=3:weights='1 0.75 0.45':normalize=0,volume=+2.8dB,alimiter=limit=0.97[SL_out];
...
[SRm][SRv][SRlate]amix=inputs=3:weights='1 0.75 0.45':normalize=0,volume=+2.8dB,alimiter=limit=0.97[SR_out];
EOF
}
...
get_clean_surround(){
cat <<'EOF'
[SL]volume=+2.3dB,alimiter=limit=0.97[SL_out];
[SR]volume=+2.3dB,alimiter=limit=0.97[SR_out];
EOF
}
...
for CUR_FILE in "${FILES[@]}"; do
  ...
  if [[ "$SONAR_MODE" = "dual" ]]; then
    BOOST="Sonar SL +2.8 dB / SR +2.8 dB  |  Clean SL +2.3 dB / SR +2.3 dB"
  else
    BOOST=$( [ "$SONAR_MODE" = "sonar" ] && echo 'Sonar SL +2.8 dB / SR +2.8 dB' || echo 'Clean SL +2.3 dB / SR +2.3 dB' )
  fi
...
FILTER_COMPLEX="[0:a:0]asplit=2[aS][aC];\
  ...
  [SLv_in_S]adelay=24|24,highpass=f=1600,equalizer=f=6500:t=q:w=1.2:g=+3.2,equalizer=f=11000:t=q:w=1.0:g=-2.0[SLv_S];\
  [SLlate_in_S]adelay=58|58,lowpass=f=1500,volume=-3dB[SLlate_S];\
  [SLm_S][SLv_S][SLlate_S]amix=inputs=3:weights='1 0.75 0.45':normalize=0,volume=+2.8dB,alimiter=limit=0.97[SL_out_S];\
  ...
  [SRm_S][SRv_S][SRlate_S]amix=inputs=3:weights='1 0.75 0.45':normalize=0,volume=+2.8dB,alimiter=limit=0.97[SR_out_S];\
  ...
  [SL_C]volume=+2.5dB,alimiter=limit=0.97[SL_out_C];\
  [SR_C]volume=+2.1dB,alimiter=limit=0.97[SR_out_C];\
