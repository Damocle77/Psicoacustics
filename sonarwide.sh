...
──────────────────────────────────────────────────────────────────────────────────────────────
UTILIZZO:
  ./sonarwide.sh <ac3|eac3> <si|no> [file|""] [bitrate] [sonar|wide] [nvidia|intel|amd|none]

ESEMPI:
  ./sonarwide.sh eac3 no film.mkv 768k sonar nvidia
  ./sonarwide.sh ac3 si "" 640k wide amd
  ./sonarwide.sh ac3 no film.mkv 448k sonar none
──────────────────────────────────────────────────────────────────────────────────────────────
USAGE
exit 1
}
...
if [[ -n "$INPUT_FILE" ]]; then
  [[ -f "$INPUT_FILE" ]] || err "File '$INPUT_FILE' non esiste"
  FILES+=("$INPUT_FILE")
else
  shopt -s nullglob
...
(( ${#FILES[@]} == 0 )) && err "Nessun file da elaborare"
...
[SRd_in]adelay=0.8,volume=1.00[SRd];
...
FILTER_COMPLEX="
[0:${A_STREAM_INDEX}]aformat=channel_layouts=5.1|5.1(side):sample_rates=48000:sample_fmts=fltp[base];
[base]channelsplit=channel_layout=${IN_LAYOUT}[FL][FR][FC][LFE][SL][SR];
${VOICE_BLOCK}
...
[FLf][FRf][FCf][LFEf][SLf][SRf]join=inputs=6:channel_layout=${IN_LAYOUT}:map=0.0-FL|1.0-FR|2.0-FC|3.0-LFE|4.0-SL|5.0-SR,
