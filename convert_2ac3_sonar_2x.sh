...
#   • neuralx → cupola ampia/spettacolare (Star Wars, Cinecomic, Transformers, F&F, Pacific Rim).
#   • atmosx  → cupola focalizzata/precisa (Alien, Blade Runner, Dune, The Batman, Tenet, Quite).
# -----------------------------------------------------------------------------------------------
...
if [[ $# -lt 3 ]] || [[ "${1:-}" == "-h" ]] || [[ "${1:-}" == "--help" ]] || [[ "${1:-}" == "help" ]]; then
  cat <<'HELP'
-----------------------------------------------------------------------------------------------  
converti_2ac3_sonar_2x.sh - TUTTO-IN-UNO
...
SIGNIFICATO
  ...
  arg2 = conserva traccia audio ORIGINALE: "si" oppure "no"
  arg3 = file di input .mkv/mp4 (prima traccia audio 5.1)
  arg4 = bitrate AC-3 (default 640k) → 320k|448k|640k
  arg5 = voicing (vale solo se arg1=sonar): "neuralx" | "atmosx"
...
# --- gestione layout audio ---
case "$IN_LAYOUT" in
  "5.1"|"5.1(side)") PAN_NORMALIZE="[0:a:0]anullref[ain];";;
  "5.1(back)")       PAN_NORMALIZE="[0:a:0]pan=FL|c0=c0,anull[aFL];[0:a:0]pan=FR|c0=c1,anull[aFR];[0:a:0]pan=FC|c0=c2,anull[aFC];[0:a:0]pan=LFE|c0=c3,anull[aLFE];[0:a:0]pan=SL|c0=c4,anull[aSL];[0:a:0]pan=SR|c0=c5,anull[aSR];[aFL][aFR][aFC][aLFE][aSL][aSR]amerge=inputs=6[ain];";;
...
[SL_out]aformat=channel_layouts=mono[SLnf];
[SR_out]aformat=channel_layouts=mono[SRnf];
...
[SL_out]aformat=channel_layouts=mono[SLaf];
[SR_out]aformat=channel_layouts=mono[SRaf];
